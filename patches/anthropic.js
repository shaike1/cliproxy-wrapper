...
export const streamAnthropic = (model, context, options) => {
    ...
    (async () => {
        ...
        try {
            ...
            // CLIPROXY NON-STREAMING PATH: Always use non-streaming for cliproxy provider
                ...
                for (const block of (nonStreamResp.content || [])) {
                    if (block.type === 'text') {
                        ...
                        output.content.push({
                            type: 'toolCall',
                            id: block.id,
                            name: isOAuthToken ? fromClaudeCodeName(block.name, context.tools) : block.name,
                            arguments: block.input || {},
                        });
                    } else if (block.type === 'thinking') {
                        output.content.push({ type: 'thinking', text: block.thinking || '' });
                    }
                }
                output.stopReason = nonStreamResp.stop_reason === 'tool_use' ? 'toolUse' :
                                    nonStreamResp.stop_reason === 'end_turn' ? 'stop' :
                                    nonStreamResp.stop_reason || 'stop';
                if (nonStreamResp.usage) {
                    output.usage.input = nonStreamResp.usage.input_tokens || 0;
                    output.usage.output = nonStreamResp.usage.output_tokens || 0;
                    output.usage.cacheRead = nonStreamResp.usage.cache_read_input_tokens || 0;
                    output.usage.cacheWrite = nonStreamResp.usage.cache_creation_input_tokens || 0;
